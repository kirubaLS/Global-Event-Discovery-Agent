...
class Settings(BaseSettings):
    ...
    # gpt-5 (reasoning): most thorough web search + verification. Runs
    # ~1-3 min at low effort; the async job + polling flow (routes_search)
    # covers the wait. Needs OpenAI credit on the account - an empty
    # balance makes the call fail/stall. Switch to gpt-4o for faster,
    # cheaper (but less thorough) runs via OPENAI_SEARCH_MODEL.
    ...
    # ── Email OTP ──────────────────────────────────────────────────
    # Master switch for the proof-of-mailbox step. On (the default) the
    # ICP form emails a 6-digit code and holds the submit until it's
    # entered; /search then requires a verified email. A sender must
    # also be configured below (Brevo / SMTP / Resend) - with none,
    # /send-verification reports {"skip": true} and the form submits
