...
async def run_gpt_event_search(profile: dict) -> dict:
    ...
    # Independent relevant-event total (rule 7b): how many qualifying
    # events GPT saw in the field, not just the 6 it picked. Clamped to
    # at least len(events) so the UI can never show "6 recommended out
    # of 4 relevant".
    try:
        candidates = int(payload.get("candidates_considered") or 0)
    except (TypeError, ValueError):
        candidates = 0
    total_relevant = max(candidates, len(events))
    ...
    return {
        ...
        "universe_stats": {
            ...
            "shows_worth_considering": total_relevant,
